 ...
 */
		...
		var AuctionHead = ( '<h2>' + daten.obj_land + ' ' + daten.obj_plz 	+ ' ' +  daten.obj_ort  + ' ' +  daten.obj_strasse + '</h2>' );
	   ...
	   // Bild Holen
		...
		AuctionTable = ('<table ><tr class="align-left"><td><ul>'			
			 ...
			 + '<li>Finanziert: ' + finanziert  + ' ' + daten.anl_whrg + ' - ' + progangelegt + ' % </li>'
...
function checkAnlage(counter) {
		...
		/*
		 * var date = new Date(Date.UTC(2012, 11, 20, 3, 0, 0)); // US
		 * englischer einsatz mit Monat-Tag-Jahr und 12-Stunden Zeit mit AM/PM
		 * console.log(date.toLocaleString('en-US')); var today = new Date();
	...
	// alert( "function=" + func + "&HP_Key=" + HP_Key + "&AnlBetrag=" +
	// AnlBetrag +
...
if (old_phpstring != PHPstring) {	var image = '';};
	    ...
	    success: function(data) {
			...
			if (daten.doc_filename === 'Bild') {			
					...
					image = image + '<lipic><button tabindex="10"><img  src="../' + savedir + daten.doc_name + '"></button></lipic>';
